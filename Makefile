PROJECTNAME ?= alertmanager-githubfiles-receiver
DESCRIPTION ?= alertmanager-githubfiles-receiver - Create and update templated files based on received Prometheus alerts. Currently specifily made for being compatible with cstate status page.
MAINTAINER  ?= Alexander Trost <galexrt@googlemail.com>
HOMEPAGE    ?= https://github.com/galexrt/alertmanager-githubfiles-receiver
...
package-%: build
	mkdir -p -m0755 $(PACKAGE_DIR)/usr/bin $(PACKAGE_DIR)/etc/sysconfig $(PACKAGE_DIR)/etc/alertmanager-githubfiles-receiver
	mkdir -p $(PACKAGE_DIR)/etc/sysconfig
	cp .build/alertmanager-githubfiles-receiver $(PACKAGE_DIR)/usr/bin
	cp srcds.example.yaml $(PACKAGE_DIR)/etc/alertmanager-githubfiles-receiver
